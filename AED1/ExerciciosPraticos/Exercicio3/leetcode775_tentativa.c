...
// Guardar os índices e os valores nos nós da árvore, contar se:
// Local: o próximo elemento é menor;
// Global: quaisquer próximos elementos menores;
// OBS: para incrementar, o índice e o valor devem ser maiores.

 int CountPermutationGlobal( no_t *raiz, int valor ) { // guardar índices também

	if ( !raiz ) return 0;

	// recursivamente passar por todo lado direito e esquerdo e ir acrescentando os valores caso a condição de inversão global seja atingida
	int permutacaoGlobalEsq = CountPermutationGlobal( raiz->esq, valor );
	int permutacaoGlobalDir = CountPermutationGlobal( raiz->dir, valor );

	return permutacaoGlobalEsq + permutacaoGlobalDir;

	// Teste:
	// return CountPermutationGlobal( raiz->esq, valor ) + CountPermutationGlobal( raiz->dir, valor );
}

 int CountPermutationLocal( no_t *raiz, int valor ) { // guardar índices também

	if ( !raiz ) return 0;

	// recursivamente passar por todo lado direito e esquerdo e ir acrescentando os valores caso a condição de inversão local seja atingida
	int permutacaoLocalEsq = CountPermutationGlobal( raiz->esq, valor );
	int permutacaoLocalDir = CountPermutationGlobal( raiz->dir, valor );

	return permutacaoLocalEsq + permutacaoLocalDir;

	// Teste:
	// return CountPermutationLocal( raiz->esq, valor ) + CountPermutationLocal( raiz->dir, valor );
}
...
bool isIdealPermutation( int *nums, int numsSize ) {
	...
	// if ( CountPermutationGlobal(  ) == CountPermutationLocal(  ) ) return true;
	// Se na função que conta a global e local retornar valores iguais, retorna verdadeiro
