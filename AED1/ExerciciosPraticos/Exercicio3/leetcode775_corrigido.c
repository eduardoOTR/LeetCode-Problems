...
static int CountSmaller( const no_t *raiz, int valor ) {
	if ( !raiz ) {
        return 0;
    }

	if ( valor < raiz->valor ) {
        return CountSmaller( raiz->esq, valor );
    }

	return Tamanho( raiz->esq ) + CountSmaller( raiz->dir, valor ) + 1;
}

bool isIdealPermutation( int *nums, int numsSize ) {
	no_t *raiz = NULL;
	long long local = 0, global = 0;
	...
	for ( int i = numsSize - 1; i >= 0; i-- ) {
		global += CountSmaller( raiz, nums[i] );
